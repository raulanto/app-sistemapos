...
export class PedidoFormStore {
  ...
  readonly canDescuentoManual = computed(() =>
    this.authService.hasPermission(...PERMISOS.ventas.descuentoManual),
  );
  ...
  readonly subtotalLocal = computed(() =>
    this.carrito().reduce((s, l) => s + this.lineaSubtotal(l), 0),
  );
  ...
  readonly faltaMotivo = computed(
    () => this.hayDescuentoManual() && !this.motivoDescuento().trim(),
  );
  ...
  readonly faltaResponsable = computed(() =>
    this.carrito().some((l) => l.esServicio && !l.asignadoA),
  );
  readonly serviciosSinResponsable = computed(
    () => this.carrito().filter((l) => l.esServicio && !l.asignadoA).length,
  );
  ...
  readonly bloques = computed(() => {
    ...
    const grupos = new Map<
      string,
      { key: string; producto: ProductoResponse; unidad: UnidadResponse | null }[]
    >();
  ...
  constructor() {
    ...
    this.usuarioService.listar({ sort: 'nombre:asc' }).subscribe({
      next: (res) =>
        this.usuarios.set(
          res.data.filter((u) => u.activo).map((u) => ({ id: u.id, nombre: u.nombre })),
        ),
      error: () => {},
    });
  ...
  private hidratar(p: PedidoResponse) {
    ...
    forkJoin(
      ids.map((id) =>
        this.productoService.obtenerPorId(id, 'unidades').pipe(catchError(() => of(null))),
      ),
    ).subscribe({
      next: (prods) => {
  ...
  setAsignado(key: string, usuarioId: string | null) {
    this.carrito.update((list) =>
      list.map((l) => (l.key === key ? { ...l, asignadoA: usuarioId || null } : l)),
    );
  }
  ...
  setPrecio(key: string, precio: number) {
    this.carrito.update((list) =>
      list.map((l) =>
        l.key === key ? { ...l, precio_unitario: Math.max(0, Number(precio) || 0) } : l,
      ),
    );
  }
  ...
  promoDeLinea(l: LineaCarrito): { etiqueta: string; descuento: number } | null {
    const m = this.cotizacion()?.lineas.find(
      (x) =>
        x.producto_id === l.producto.id &&
        (x.producto_unidad_id ?? null) === (l.unidad?.id ?? null),
    );
  ...
  guardar(confirmar = false) {
    if (!this.puedeGuardar()) {
      if (this.faltaDireccion())
        this.sonner.error('La dirección es obligatoria para envío a domicilio.');
    ...
    this.pedidoService.crear(req, this.idemKey).subscribe({
      next: (p) =>
        this.alGuardar(p, confirmar ? 'Pedido creado y confirmado' : 'Pedido guardado en borrador'),
      error: (err) => this.errorGuardar(err),
    });
  }
  ...
  private errorGuardar(err: unknown) {
    ...
    if (code === 'DireccionEnvioRequerida')
      this.sonner.error('La dirección es obligatoria para envío a domicilio.');
    else if (code === 'MotivoDescuentoRequerido')
    ...
    else if (code === 'PedidoNoEditable')
      this.sonner.error('El pedido ya no está en borrador. Usa «Reabrir» para editarlo.');
    else if (code === 'ServicioSinResponsable')
      this.sonner.error('Asigna un responsable a cada línea de servicio antes de confirmar.');
    else if (code === 'ResponsableInvalido')
      this.sonner.error(
        'El responsable elegido no es un usuario activo, o esa línea no es un servicio.',
      );
    else if (code === 'TransicionPedidoInvalida')
      this.sonner.error('Ese cambio de estado no está permitido ahora.');
    else
      this.sonner.error(
        mensajePedidoError(e?.error?.error?.message, 'No se pudo guardar el pedido'),
      );
  }
}
